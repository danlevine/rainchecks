...
class Item extends Component {
  ...
  render() {
    ...
    const castStr = cast ? cast.slice(0, 3).map(x=>x.name).join(', ') : '';
    const directorStr = crew ? crew.filter(x=>x.job === "Director")[0].name : '';
    const expandedClassStr = this.state.itemExpanded ? ' item-expanded' : ' item-collapsed';
      ...
      <li className={`item${expandedClassStr}`}>
        ...
        <div className="item__info-block">
          <div className="item__label">
            ...
            <p className="item__tech-info">{releaseDateStr} &middot; {runtime} min</p>
          </div>
